...
impl From<emitter::EmitterError> for Error {
    fn from(err: emitter::EmitterError) -> Self {
        match err {
            emitter::EmitterError::Libyaml(err) => Self::from(err),
            emitter::EmitterError::Io(err) => new(ErrorImpl::Io(err)),
        }
    }
}
